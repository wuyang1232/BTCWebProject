...
func (a AddressInfo)SaveGetAddressInfoData()(int64,error)  {
	//插入数据
	result,err :=mysql.DB.Exec("insert into getaddressinfo(address,scriptPubKey,ismine,solvable,desc,iswatchonly,isscript,iswitness,witness_version,witness_program,pubkey,ischange,timestamp,hdkeypath,hdseedid,hdmasterfingerprint)values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
...
func (a AddressInfo) QueryGetAddressInfoData() (*AddressInfo,error){
	row := mysql.DB.QueryRow("select address,scriptPubKey,ismine,solvable,desc,iswatchonly,isscript,iswitness,witness_version,witness_program,pubkey,ischange,timestamp,hdkeypath,hdseedid,hdmasterfingerprint from getaddressinfo where address = ?",
		a.Address)
	err := row.Scan(&a.Address,&a.ScriptPubkey,&a.Ismine,&a.solvable,&a.Desc,&a.Iswatchonly,&a.Isscript,&a.Iswitness,&a.Witness_version,&a.Witness_program,&a.Pubkey,
		a.Ischange,&a.Timestamp,&a.Hdkeypath,&a.Hdseedid,&a.Hdmasterfingerprint)
	if err != nil {
